...
contract ProtocolSettings is State, ProtocolSettingsEvents {
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < tokens.length; i++) {
            IBEP20(tokens[i]).safeApprove(router, 0);
            IBEP20(tokens[i]).safeApprove(router, uint256(-1));
        }
    }
        ...
        onlyOwner
    {
        ...
        require(IBEP20(nbuTokenAddress).transferFrom(
            msg.sender,
            address(this),
            amount
        ), "ProtocolSettings: Error transferring");
    ...
    function updateProtocolTokenHeld()
        ...
        onlyOwner
    {
        protocolTokenHeld = IBEP20(nbuTokenAddress).balanceOf(address(this));
    }
