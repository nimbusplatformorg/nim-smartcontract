...
contract LoanClosingsBase is State, LoanClosingsEvents, VaultController, InterestUser, GasTokenUser, SwapsUser, LiquidationHelper {
    ...
    // The receiver always gets back an BEP20 (even WBNB)
        ...
        internal
    {
        if (principalNeeded != 0) {
            if (msg.value == 0) {
                ...
                require(loanToken == address(wbnbToken), "wrong asset sent");
                require(msg.value >= principalNeeded, "not enough BNB");
                wbnbToken.deposit.value(principalNeeded)();
    ...
    {
        if (assetAmount != 0) {
            if (assetToken == address(wbnbToken)) {
                vaultBnbWithdraw(
                    receiver,
                    assetAmount
                );
