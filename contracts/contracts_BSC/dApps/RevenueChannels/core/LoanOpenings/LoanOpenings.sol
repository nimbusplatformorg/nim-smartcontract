...
contract LoanOpenings is State, LoanOpeningsEvents, VaultController, InterestUser, SwapsUser {
    ...
    function borrowOrTradeFromPool(
        ...
        uint256[5] calldata sentValues,
            ...
            // protocolLoanInterest: new amount of interest to escrow for loan (determines initial loan length)
            // loanTokenReceived: total loanToken deposit (amount not sent to borrower in the case of loans)
    ...
    {
        require(msg.value == 0 || loanDataBytes.length != 0, "loanDataBytes required with BNB");
        ...
        if (initialMargin == 0) {
            require(isProtocolLoan, "initialMargin == 0");
        ...
        returns (uint256 collateralAmountRequired)
    {
        if (marginAmount != 0) {
            ...
            uint256 feePercent = isProtocolLoan ?
                borrowingFeePercent :
                tradingFeePercent;
        ...
        returns (uint256 collateralAmountRequired)
    {
        ...
        return getRequiredCollateral(
            ...
            loanParamsLocal.maxLoanTerm == 0 ? // isProtocolLoan
                true :
                false
        );
    }
    ...
    function _borrowOrTrade(
        ...
        uint256[5] memory sentValues,
            // newRate: new loan interest rate
            // newPrincipal: new loan size (borrowAmount + any borrowed interest)
            // protocolLoanInterest: new amount of interest to escrow for protocol loan (determines initial loan length)
        ...
        returns (LoanOpenData memory)
    {
        ...
        // maxLoanTerm == 0 indicates a protocol loan and requres that protocolLoanInterest != 0
        require(loanParamsLocal.maxLoanTerm != 0 ||
            sentValues[2] != 0, // protocolLoanInterest
            "invalid interest");
        ...
        uint256 amount = _initializeInterest(
            ...
            sentValues[2]  // protocolLoanInterest
        );
    ...
    function _initializeInterest(
        ...
        uint256 newRate,
        uint256 newPrincipal,
        uint256 protocolLoanInterest) // ignored for fixed-term loans
        internal
        returns (uint256 interestAmountRequired)
    {
        ...
        if (maxLoanTerm == 0) {
            // indefinite-term (protocol) loan

            // protocolLoanInterest != 0 was confirmed earlier
            loanLocal.endTimestamp = protocolLoanInterest
            ...
            interestAmountRequired = protocolLoanInterest;
        } else {
            // fixed-term loan
        ...
        returns (uint256 collateralTokenAmount)
    {
        ...
        if (isProtocolLoan && collateralTokenAmount != 0) {
