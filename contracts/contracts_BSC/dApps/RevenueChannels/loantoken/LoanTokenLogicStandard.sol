...
contract LoanTokenLogicStandard is AdvancedToken, GasTokenUser {
    ...
    address public constant wbnbToken = 0xd0A1E359811322d97991E03f863a0C30C2cF029C; // ropsten and mainnet
        ...
        returns (bytes memory)
    {
        ...
        uint256 beforeBnbBalance = address(this).balance.sub(msg.value);
        ...
        require(
            address(this).balance >= beforeBnbBalance &&
            _underlyingBalance()
    ...
    function borrow(
        ...
        uint256 collateralTokenSent,    // if 0, loanId must be provided; any BNB sent must equal this value
        address collateralTokenAddress, // if address(0), this means BNB and BNB must be sent with the call or loanId must be provided
        address borrower,
        address receiver,
        ...
        returns (bool)
    {
        return _internalTransferFrom(
            ...
            /*ProtocolLike(revenueChannelsProtocol).isLoanPool(msg.sender) ?
                uint256(-1) :
                allowed[_from][msg.sender]*/
        );
    }
    ...
    function totalAssetBorrow()
        ...
        returns (uint256)
    {
        return ProtocolLike(revenueChannelsProtocol).getTotalPrincipal(
            address(this),
            loanTokenAddress
        );
    }
        ...
        returns (uint256 principal, uint256 collateral, uint256 interestRate, uint256 collateralToLoanRate)
    {
        ...
        collateral = ProtocolLike(revenueChannelsProtocol).getEstimatedMarginExposure(
            loanTokenAddress,
            collateralTokenAddress,
            loanTokenSent,
            collateralTokenSent,
        ...
        returns (uint256) // depositAmount
    {
        if (borrowAmount != 0) {
            ...
            if (newBorrowAmount <= _underlyingBalance()) {
                ...
                return ProtocolLike(revenueChannelsProtocol).getRequiredCollateralByParams(
        ...
        returns (uint256 borrowAmount)
    {
        if (depositAmount != 0) {
            ...
            borrowAmount = ProtocolLike(revenueChannelsProtocol).getBorrowAmountByParams(
                loanParamsIds[uint256(keccak256(abi.encodePacked(
                    collateralTokenAddress,
                    true
                )))],
        ...
        returns (uint256 mintAmount)
    {
        ...
        if (msg.value == 0) {
            ...
            IWbnb(wbnbToken).deposit.value(depositAmount)();
        }
        ...
        returns (ProtocolLike.LoanOpenData memory loanOpenData)
    {
        ...
        ProtocolLike(revenueChannelsProtocol).setDepositAmount(
            loanOpenData.loanId,
            totalDeposit,
            totalDeposit
                .mul(WEI_PRECISION)
        ...
        internal
    {
        ...
        if (lastSettleTime_ != ts) {
            ProtocolLike(revenueChannelsProtocol).withdrawAccruedInterest(
                loanTokenAddress
            );
        ...
        returns (uint256 totalDeposit, uint256 collateralToLoanRate)
    {
        ...
        (collateralToLoanRate, collateralToLoanPrecision) = FeedsLike(ProtocolLike(revenueChannelsProtocol).priceFeeds()).queryRate(
            collateralTokenAddress,
            loanTokenAddress
        );
        ...
        returns (ProtocolLike.LoanOpenData memory)
    {
        ...
        bool isProtocolLoan = withdrawAmount != 0 ?
            true :
            false;
        ...
        return ProtocolLike(revenueChannelsProtocol).borrowOrTradeFromPool.value(msgValue)(
            loanParamsId,
            loanId,
            isProtocolLoan,
        ...
        returns (uint256 msgValue)
    {
        ...
        if (withdrawalAmount != 0) { // withdrawOnOpen == true
            ...
            if (newPrincipal > withdrawalAmount) {
                _safeTransfer(_loanTokenAddress, revenueChannelsProtocol, newPrincipal - withdrawalAmount, "27");
            }
        } else {
            _safeTransfer(_loanTokenAddress, revenueChannelsProtocol, newPrincipal, "27");
        }

        if (collateralTokenSent != 0) {
            if (collateralTokenAddress == _wbnbToken && msgValue != 0 && msgValue >= collateralTokenSent) {
                IWbnb(_wbnbToken).deposit.value(collateralTokenSent)();
                _safeTransfer(collateralTokenAddress, revenueChannelsProtocol, collateralTokenSent, "28");
                msgValue -= collateralTokenSent;
            } else {
                _safeTransferFrom(collateralTokenAddress, msg.sender, revenueChannelsProtocol, collateralTokenSent, "28");
            }
        }

        if (loanTokenSent != 0) {
            _safeTransferFrom(_loanTokenAddress, msg.sender, revenueChannelsProtocol, loanTokenSent, "29");
        }
    }
        ...
        internal
    {
        _callOptionalReturn(
            token,
            abi.encodeWithSelector(IBEP20(token).transfer.selector, to, amount),
            errorMsg
        );
    }
        ...
        internal
    {
        _callOptionalReturn(
            token,
            abi.encodeWithSelector(IBEP20(token).transferFrom.selector, from, to, amount),
            errorMsg
        );
    }
    ...
    function _underlyingBalance()
        ...
        returns (uint256)
    {
        return IBEP20(loanTokenAddress).balanceOf(address(this));
    }
        ...
        returns (uint256)
    {
        if (assetBorrow != 0 && assetSupply >= assetBorrow) {
            return _avgBorrowInterestRate(assetBorrow)
                ...
                .mul(SafeMath.sub(WEI_PERCENT_PRECISION, ProtocolLike(revenueChannelsProtocol).lendingFeePercent()))
            ...
            uint256 interestUnPaid)
    {
        ...
        (,,interestOwedPerDay,interestUnPaid,interestFeePercent,) = ProtocolLike(revenueChannelsProtocol).getLenderInterestData(
            address(this),
            loanTokenAddress
        );
