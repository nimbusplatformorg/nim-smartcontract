...
contract SmartLP is SmartLPStorage, IBEP721, IBEP721Metadata {
    ...
    function buySmartLP(uint amount) payable external {
      ...
      uint swapAmount = amountBusd/6;
      tokenCount = ++tokenCount;
      
      address[] memory path = new address[](2);
      path[0] = address(busdToken);
      path[1] = address(nbuToken);
      (uint[] memory amountsBusdNbuSwap) = swapRouter.swapExactTokensForTokens(swapAmount * 2, 0, path, address(this), block.timestamp);

      path[1] = address(WBNB);      
      (uint[] memory amountsBusdBnbSwap) = swapRouter.swapExactTokensForBNB(swapAmount, 0, path, address(this), block.timestamp);
      
      amountBusd -= swapAmount * 3;
      
      (, uint amountBnbNbu, uint liquidityBnbNbu) = swapRouter.addLiquidityBNB{value: amountsBusdBnbSwap[0]}(address(nbuToken), amountsBusdNbuSwap[1]/2, 0, 0, address(this), block.timestamp);
      // amountBNB -= amountBnbNbu;
      
      (, uint amountBusdNbu, uint liquidityBusdNbu) = swapRouter.addLiquidity(address(busdToken), address(nbuToken), amountsBusdNbuSwap[0]/2, amountsBusdNbuSwap[1]/2, 0, 0, address(this), block.timestamp);
      ...
      uint amountRewardEquivalentBusdNbu = lpStakingNbuBusd.getCurrentLPPrice() * liquidityBusdNbu / 1e18;
      _balancesRewardEquivalentNbuBusd[tokenCount] += amountRewardEquivalentBusdNbu;
      ...
      userSupply.PoolBnbAmount = amountsBusdBnbSwap[1];
