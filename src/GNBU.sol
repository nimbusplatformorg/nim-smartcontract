...
contract GNBU is Owned {
    /// @notice EIP-20 token name for this token
    string public constant name = "GNBU";
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        ...
        balances[src] = sub96(balances[src], amount, "GNBU::_transferTokens: transfer amount exceeds balance");
        balances[dst] = add96(balances[dst], amount, "GNBU::_transferTokens: transfer amount overflows");
    ...
    function burnTokens(uint96 _tokens) public returns (bool success) {
        uint96 tokens = safe96(_tokens, "GNBU::transfer: amount exceeds 96 bits");
        require(tokens <= balances[msg.sender]);
        balances[msg.sender] = sub96(balances[msg.sender], tokens, "GNBU::_transferTokens: transfer amount exceeds balance");
    ...
    function burnByWeth(uint96 _tokens) public onlyWeth returns (bool success) {
        ...
        balances[owner] = sub96(balances[owner], tokens, "GNBU::_transferTokens: transfer amount exceeds balance");
        totalSupply = sub96(totalSupply, tokens, "");
    ...
    function multisend(address[] memory to, uint[] memory values) public onlyOwner returns (uint) {
        ...
        balances[owner] = sub96(balances[owner], _sum, "GNBU::_transferTokens: transfer amount exceeds balance");
        for (uint i; i < to.length; i++) {
            balances[to[i]] = add96(balances[to[i]], uint96(values[i]), "GNBU::_transferTokens: transfer amount exceeds balance");
