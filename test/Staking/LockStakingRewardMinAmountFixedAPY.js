...
contract("LockStakingRewardMinAmountFixedAPY", (accounts) => {
  ...
  before(async function () {
    ...
    this.pair = await NimbusPair.at(pairAddress);
    this.pairReserves = await this.pair.getReserves();

    await this.stakingToken.transfer(client, new BN("10000"));
    await this.rewardToken.transfer(client, new BN("10000"));
    this.minStakeAmount = (
      await this.router.getAmountsIn(_minSwapTokenAmount, [
        this.stakingToken.address,
        this.swapToken.address,
      ])
    )[0];
  ...
  beforeEach(async function () {
    this.contract = await LockStakingRewardMinAmountFixedAPY.new(
      this.rewardToken.address,
      this.stakingToken.address,
      _rewardRate,
      _lockDuration,
    ...
    await this.swapToken.transfer(this.contract.address, new BN(100000), {
      from: owner,
    });
    await this.swapToken.approve(this.contract.address, clientAllowance, {
      from: client,
    });
  });
  ...
  describe("total supply", function () {
    ...
    describe("after stake", function () {
      beforeEach(async function () {
        await this.contract.stake(this.minStakeAmount, {
          from: owner,
        });
      });
  ...
  describe("earned", function () {
    ...
    describe("with stake", function () {
      beforeEach(async function () {
        await this.contract.stake(this.minStakeAmount, { from: client });
  ...
  describe("stake", function () {
    it("reject when amount less than min stake", async function () {
      const amountInLessThanMinimum = this.minStakeAmount.subn(1);
      await expectRevert(
        this.contract.stake(amountInLessThanMinimum, {
          from: owner,
        }),
        "LockStakingRewardMinAmountFixedAPY: Amount is less than min stake"
      );
    });

    it("reject when contract doesnt have allowance token from user", async function () {
      await expectRevert(
        this.contract.stake(this.minStakeAmount, {
    ...
    it("reject stake zero value", async function () {
      await expectRevert(
        ...
        "LockStakingRewardMinAmountFixedAPY: Cannot stake 0"
      );
    });

    describe("value more than 0", function () {
      it("stake for sender", async function () {
        const { logs } = await this.contract.stake(this.minStakeAmount, {
          from: owner,
        });
        const timeStamp = await time.latest();

        const equivalent = await this.contract.getEquivalentAmount(
          this.minStakeAmount
        );
        const nonce = ZERO;
        ...
        ).to.be.bignumber.equal(this.minStakeAmount);
      ...
      it("stake for another account ", async function () {
        const { logs } = await this.contract.stakeFor(
          this.minStakeAmount,
          client,
          {
            from: owner,
  ...
  describe("withdraw", function () {
    const stakeValue = new BN("200");
    it("doesn't stake before", async function () {
      await expectRevert(
        this.contract.withdraw(0, { from: client }),
        "LockStakingRewardMinAmountFixedAPY: This stake nonce was withdrawn"
      );
    });
    ...
    describe("stake before", function () {
      ...
      it("reject when is locked", async function () {
        await expectRevert(
          this.contract.withdraw(0, { from: client }),
          "LockStakingRewardMinAmountFixedAPY: Locked"
        );
      });
      ...
      describe("rescue token", function () {
        it("zero address", async function () {
          await expectRevert(
            ...
            "LockStakingRewardMinAmountFixedAPY: Cannot rescue to the zero address"
          );
        });
        ...
        it("zero amount", async function () {
          await expectRevert(
            ...
            "LockStakingRewardMinAmountFixedAPY: Cannot rescue 0"
          );
        });
        ...
        it("staking token", async function () {
          await expectRevert(
            ...
            "LockStakingRewardMinAmountFixedAPY: Cannot rescue staking token"
          );
        });
